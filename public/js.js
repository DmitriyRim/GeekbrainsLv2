...
const map = {
  ...
  render(snakePointsArray, foodPoint) {
    ...
    this.sayScore(snakePointsArray.length-1);
  },
  sayScore(sizeSnakeBody){
    const score = document.getElementById('score');
    score.textContent = sizeSnakeBody;
  }
};
...
const snake = {
  ...
  getNextStepHeadPoint() {
    ...
    switch (this.direction) {
      case 'up':
        return firstPoint.y === 0 ? {x: firstPoint.x, y: config.getRowsCount()-1} :{x: firstPoint.x, y: firstPoint.y - 1};
      case 'right':
        return firstPoint.x === config.getRowsCount()-1 ? {x: 0, y: firstPoint.y} : {x: firstPoint.x + 1, y: firstPoint.y};
      case 'down':
        return firstPoint.y === config.getRowsCount()-1 ? {x: firstPoint.x, y: 0} : {x: firstPoint.x, y: firstPoint.y + 1};
      case 'left':
        return firstPoint.x === 0 ? {x: config.getRowsCount()-1, y: firstPoint.y} : {x: firstPoint.x - 1, y: firstPoint.y};
    }
  },
